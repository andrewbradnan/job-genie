...
object bayes {
    ...
    def countStrings(strs: List[String]) = {
        var clean = strs.filterNot(s => s.length < 4)	// remove strings 3 or less
        clean = clean.filterNot(isAllDigits(_))			// remove numbers
        clean = clean.map(_.toLowerCase)
        clean.filter(word => !stop_words.exists(word == _)).
		foldLeft(counts) {
